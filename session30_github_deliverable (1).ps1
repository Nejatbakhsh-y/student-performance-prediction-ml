...
heading = "Session 30 — Neural-Network Regression"
nb.cells = [
...
mlp_test_metrics = {
    ...
    "R2": float(metric_lookup.get("R2", metric_lookup.get("R²"))),
}
...
'@
    ...
    if (-not $SkipExecution) {
        ...
        $VerificationCode = "import nbformat,sys; n=nbformat.read(sys.argv[1],as_version=4); s='\n'.join(str(o.get('text','')) for c in n.cells for o in c.get('outputs',[])); assert 'SESSION 30 NOTEBOOK VERIFICATION PASSED' in s; print('Notebook output verification passed.')"

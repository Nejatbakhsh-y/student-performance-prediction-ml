...
if ($OriginalStatus.Count -gt 0) {
    Write-Host ($OriginalStatus -join "`n") -ForegroundColor Yellow
    throw "The repository has uncommitted changes. Commit or resolve them before running Session 37."
}
...
$LocalBranchExists = (& git branch --list $BranchName).Trim()
if ($CurrentBranch -ne $BranchName) {
    if (-not [string]::IsNullOrWhiteSpace($LocalBranchExists)) {
...
Invoke-Git add -- $NotebookRelative "reports/evidence/session37_github_deliverable.md" "scripts/08_session37_github_deliverable.ps1"
